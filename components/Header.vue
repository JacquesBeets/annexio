<template>
    <div class="header flex justify-between items-center mb-5 p-3 shadow-md bg-gray-300 dark:bg-gray-800">
        ...
        <div class="rightNav flex">
            ...
            <!-- Search -->
            <div class="flex items-center justify-center bg-gray-800 mr-3 rounded-md">
                <form method="GET">
                    ...
                    <input type="search" name="q" class="py-2 text-sm text-white bg-gray-900 rounded-md pl-10 focus:outline-none focus:bg-white focus:text-gray-900" placeholder="Search Country..." autocomplete="off" v-model="searchSelection">
                    </div>
                </form>
            </div>
            <!-- Dark Mode -->
...
export default Vue.extend({
    ...
    computed:{
        ...
        searchSelection:{
            get(){
                return this.$store.state.countrySearch
            },
            set(value){
                this.$store.commit("setCountrySearch", value)
            }
        }
    },
    methods:{
